...
_(Synthesized from the entire design conversation – free of source code, ready for peer-level engineering review.)_
...
| **`orchestrator_lite`** (in `agent/`) | Small LLM (`gpt-4o-mini`) that performs safety moderation _and_ intent extraction. Returns JSON with `{allowed, intent, args}`. | Replaces earlier ML-filter idea.          |
| **`agent_core`** (in `agent/`)        | Builds `SecureAgent`: registers tools, runs full LLM (`gpt-4o`) in ReAct mode when orchestrator decides a complex question.     | Stores full reasoning scratchpad in logs. |
| **`chat_interface`**                  | `cli_chat` (Rich TTY) and optional `demo_streamlit`.                                                                            | Consume `agent_core`.                     |
